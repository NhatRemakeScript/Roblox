...
-- ================== ANTI SERVER KICK ==================
spawn(function()
    -- Hook các Remote có thể gây kick
    for _, remote in ipairs(RS.Remotes:GetDescendants()) do
        if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") then
            local name = remote.Name:lower()
            if name:find("kick") or name:find("ban") or name:find("cheat") or name:find("detect") or name:find("anti") then
                -- Vô hiệu hóa remote kick
                if remote:IsA("RemoteEvent") then
                    remote.FireServer = function() return nil end
                elseif remote:IsA("RemoteFunction") then
                    remote.InvokeServer = function() return nil end
                end
                print("🛡️ Đã chặn remote:", remote.Name)
            end
        end
    end
    
    -- Xóa thông báo kick nếu có
    Player:WaitForChild("PlayerGui").ChildAdded:Connect(function(child)
        if child.Name:lower():find("kick") or child.Name:lower():find("ban") or child.Name:lower():find("cheat") or child.Name:lower():find("removed") then
            wait(0.1)
            child:Destroy()
            print("🛡️ Đã xóa GUI kick:", child.Name)
        end
    end)
end)
...
TT.Text = "⚡ DucNhat 2.0.6 ANTI-KICK"
...
local function CreateToggle(parent, text, def, cb)
    ...
    local function setState(ns)
        ...
        TS:Create(knob, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {Position = targetPos, Rotation = targetRot}):Play()
